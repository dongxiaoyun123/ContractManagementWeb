        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              ...
              @click="SecondClick(scope.row.ContractCode)"
            />
          </template>
        </el-table-column>
        <el-table-column
          v-else
          key="ContractCodeFalse"
          prop="ContractCode"
          label="合同编号"
          width="200"
          show-overflow-tooltip
      ...
      >
        <el-row>
          <el-col :span="12">
            ...
            >
              <el-input
                v-model="addSupplierForm.ContractCode"
                placeholder="系统自动生成"
                disabled
              />
            </el-form-item>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="公司名称" prop="CompanyId">
              <el-select
                v-model="addSupplierForm.CompanyId"
                filterable
                placeholder="合同类型"
                clearable=""
              >
          ...
          <el-col :span="12">
            <el-form-item label="合同类型" prop="ContractType">
              <el-select
                v-model="addSupplierForm.ContractType"
                filterable
                placeholder="合同类型"
                clearable=""
              >
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item ref="BeginTime" label="开始日期" prop="BeginTime">
              <el-date-picker
                v-model="addSupplierForm.BeginTime"
                type="date"
                placeholder="选择日期"
              />
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="结束日期" prop="EndTime">
              <el-date-picker
                v-model="addSupplierForm.EndTime"
                type="date"
                placeholder="结束日期"
              />
            </el-form-item>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="乙方公司" prop="SupplierCmpanyBCode">
              ...
              <el-select
                v-model="addSupplierForm.SupplierCmpanyBCode"
              ...
              >
                <el-option
                  v-for="item in selectSupplierCmpanyBList"
                  :key="item.value"
                  :label="item.label"
                  :value="item.value"
                />
      ...
      >
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="公司名称" prop="CompanyId">
              <el-select
                v-model="updateSupplierFrom.CompanyId"
                :disabled="ConCompany"
                filterable
                placeholder="合同类型"
                clearable=""
          ...
          <el-col :span="12">
            <el-form-item label="合同类型" prop="ContractType">
              <el-select
                v-model="updateSupplierFrom.ContractType"
                filterable
                placeholder="合同类型"
                clearable=""
              >
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item ref="BeginTime" label="开始日期" prop="BeginTime">
              <el-date-picker
                v-model="updateSupplierFrom.BeginTime"
                type="date"
                placeholder="选择日期"
              />
            </el-form-item>
          ...
          <el-col :span="12">
            <el-form-item label="结束日期" prop="EndTime">
              <el-date-picker
                v-model="updateSupplierFrom.EndTime"
                type="date"
                placeholder="结束日期"
              />
            </el-form-item>
        ...
        <el-row>
          <el-col :span="12">
            <el-form-item label="乙方公司" prop="SupplierCmpanyBCode">
              <!-- <el-select
                v-model="updateSupplierFrom.SupplierCmpanyBCode"
                class="CompanySelectClass"
                filterable
                placeholder="乙方公司"
                clearable=""
              >
                <el-option
                  v-for="item in SupplierCmpanyBList"
                  :key="item.SupplierCmpanyBCode"
                  :label="item.SupplierCmpanyBName"
                  :value="item.SupplierCmpanyBCode"
                />
              </el-select> -->
              <el-select
                v-model="updateSupplierFrom.SupplierCmpanyBCode"
        ...
        <el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item label="备注" prop="Remark">
                <el-input
                  v-model="updateSupplierFrom.Remark"
                  type="textarea"
                  :rows="5"
                  maxlength="500"
                />
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="已上传文件">
                <el-button
                  v-if="updateSupplierFrom.ArchivedType == 2"
                ...
                >归档</el-button>
                <div
                  v-for="item in updateSupplierFrom.FileLists"
                  :key="item.name"
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item label="附件上传">
                ...
                >
                  <el-button
                    v-if="updateSupplierFrom.ArchivedType != 3"
                    slot="trigger"
                    icon="el-icon-position"
                    size="mini"
                    plain
                ...
                <el-button
                  v-if="
                    updateSupplierFrom.ArchivedType != 3 &&
    ...
    >
      ...
      >
        ...
        <el-row class="buttonCenter">
          <el-col>
            <el-button
              v-loading.fullscreen.lock="LoadingAddSupplier"
              icon="el-icon-circle-check"
              type="primary"
              @click="saveAdd"
            >确 定</el-button>
...
export default {
  ...
  methods: {
    ...
    saveAuditStatusCheck() {
      ...
      SaveAuditStatusSupplier(this.Id, this.auditStatusChange).then((res) => {
        if (res.success) {
    ...
    isRed({ row }) {
      const checkIdList = this.multipleSelection.map(
        (item) => item.CreationTimeStr + item.ContractCode
      );
      if (checkIdList.includes(row.CreationTimeStr + row.ContractCode)) {
        return {
    ...
    addIfShowUploadButton() {
      ...
      if (
        ...
        this.addSupplierForm.FileList.length != 0
      ) {
    ...
    AttachmentCodeRemove(file, fileList) {
      ...
      this.addSupplierForm.FileList = this.addSupplierForm.FileList.filter(
        (f) => {
    ...
    updateIfShowUploadButton() {
      ...
      if (
        ...
        this.updateSupplierFrom.FileList.length != 0
      ) {
      ...
      if (
        ...
        for (const index in this.updateSupplierFrom.FileList) {
          var flagf = this.fileListUpload.filter((item) => {
            return (
              item.name == this.updateSupplierFrom.FileList[index].FileName
            );
          });
    ...
    AttachmentCodeRemoveUpdate(file, fileList) {
      ...
      this.updateSupplierFrom.FileList =
        this.updateSupplierFrom.FileList.filter((f) => {
          return addNew.includes(f.FileName);
        });
      this.updateIfShowUploadButton();
    },
        ...
        if (!valid) {
          ...
          if (
            ...
            for (const index in this.addSupplierForm.FileList) {
              var flagf = this.fileList.filter((item) => {
                return (
                  item.name == this.addSupplierForm.FileList[index].FileName
                );
              });
          ...
          this.addSupplierForm.EndTime = this.addSupplierForm.EndTime
            ...
            : "";
          this.addSupplierForm.CompanyId = this.addSupplierForm.CompanyId + "";
    ...
    saveUpdate() {
      ...
      this.$refs.updateContractsRef.validate(async (valid, object) => {
        ...
        if (!valid) {
          ...
          if (
            this.updateSupplierFrom.BeginTime >= this.updateSupplierFrom.EndTime
          ) {
            this.LoadingUpdate = false;
            this.$message.warning("开始时间不能大于等于结束时间，请检查");
            return;
    ...
    _getContractData() {
      ...
      var parameter = {
        ...
        ArchivedType: this.ArchivedType || null,
        SupplierCmpanyBCode: this.SupplierCmpanyBCode,
        PageIndex: this.queryInfo.pagenum,
        PageSize: this.queryInfo.pagesize,
      };
      GetSupplierData(parameter).then((res) => {
        if (res.success) {
    ...
    exportSupplier() {
      ...
      GetSupplierDataExport(parameter).then((res) => {
        this.fakes.end();
        // 初始化进度条
        setTimeout(() => {
    ...
    // 编辑获取反填信息
    showEditDialog(item) {
      this.remoteMethodShow(item.SupplierCmpanyBCode);
      this.UploadServerFlagUpdate = false;
    ...
    // 反填只显示选中的公司名称
    remoteMethodShow(SupplierCmpanyBCode) {
      setTimeout(() => {
        GetSupplierCmpanyById(SupplierCmpanyBCode).then((res) => {
          if (res.success) {
            this.SupplierCmpanyBList = [];
            // 公司数据格式化
            this.SupplierCmpanyBList = res.result.list.map((item) => {
              return {
                value: item.SupplierCmpanyBCode,
                label: item.SupplierCmpanyBName,
              };
            });
            this.selectSupplierCmpanyBList = this.SupplierCmpanyBList.filter(
              (item) => {
                return item.value == SupplierCmpanyBCode;
              }
            );
          } else {
            this.SupplierCmpanyBList = [];
            this.selectSupplierCmpanyBList = [];
          }
        });
      }, 200);
    },
    ...
    // 获取乙方公司数据
    GetSupplierCmpanyB() {
      GetSupplierCmpanyB("", 1, 100000).then((res) => {
        if (res.success) {
    ...
    remoteMethod(query) {
      if (query && query.length > 2) {
        setTimeout(() => {
          GetSupplierCmpanyB(query, 1, 100000).then((res) => {
            if (res.success) {
              this.SupplierCmpanyBList = [];
              // 公司数据格式化
              this.SupplierCmpanyBList = res.result.list.map((item) => {
