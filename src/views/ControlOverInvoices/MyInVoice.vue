  ...
  <div style="padding: 16px;">
    ...
    <el-card class="CardTableClass">
      <el-table v-loading="loading" class="tableCheckClass" :data="InvoiceData" fit ref="multipleTable"
              ...
              <div slot="content">
                <div style="display: flex;  align-items: center;">
                  <span slot="reference" style="margin: 0 10px 0 6px;" class="SecondPartyNameClass">
                    ...
                    <div><el-tag key="未确认" effect="plain" type="">未确认</el-tag></div>
        ...
        <el-table-column prop="InvoiceRemark" label="发票备注" align="left" min-width="200" show-overflow-tooltip />
      </el-table>
      <!-- 分页区域 -->
  ...
  methods: {
    ...
    TableSelect(selection) {
      ...
      if (selection.length != 0) {
        ...
        this.IdStr =
          this.IdStr.substring(0, this.IdStr.lastIndexOf(",")
          );
      } else {
        this.IdStr = "";
      }
      ...
      GetDicCategory("InvoiceStatus").then((res) => {
        if (res.success) {
          res.result = res.result.filter((item) => {
            return item.Code == "1" || item.Code == "0" || item.Code == "4" || item.Code == "5" || item.Code == "7"
          })
