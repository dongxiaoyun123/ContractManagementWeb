...
export default {
  ...
  watch: {
    ...
    clickRow: {
      handler() {
        this.GetSupplierOtherOrderById();
      },
  ...
  methods: {
    ...
    GetSupplierOtherOrderById() {
      this.loading = true;
      GetSupplierOtherOrderById(this.clickRow.InsProductPayCode).then((res) => {
        this.loading = false;
        if (res.success) {
      ...
      if (columnIndex === 1) {
        ...
        if (
          ...
          row.ContractOrderName !==
            this.CollectionOrderData[rowIndex - 1].ContractOrderName
        ) {
        ...
        if (
          rowIndex === 0 ||
          row.ApplicationTime !==
            this.CollectionOrderData[rowIndex - 1].ApplicationTime ||
          row.ContractOrderName !==
            this.CollectionOrderData[rowIndex - 1].ContractOrderName
        ) {
          const rowspan = this.CollectionOrderData.filter(
        ...
        if (
          rowIndex === 0 ||
          row.CompanyName !==
            this.CollectionOrderData[rowIndex - 1].CompanyName ||
          row.ApplicationTime !==
            this.CollectionOrderData[rowIndex - 1].ApplicationTime ||
          row.ContractOrderName !==
            this.CollectionOrderData[rowIndex - 1].ContractOrderName
