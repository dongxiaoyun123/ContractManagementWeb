...
/**
* 获取日期所在月份的第一天和最后一天（传入字符串）
* @param {HTMLElement} elm
* @param {string} cls
*/
export function getDateByTimesLast(Times) {
  var date = new Date(Times) // this.startTime = '2022-5-02'
...
/**
* 获取当前日期所在的年月日格式化（year 输出年 month 输出年-月 day 输出年-月-日）
*/
export function getNowDate(type) {
  ...
  switch (type) {
    case "year":
      ...
      break;
    case "month":
      NOW_MONTHS_AGO = `${year}-${month}`;
      break;
    case "day":
      NOW_MONTHS_AGO = `${year}-${month}-${day}`;
      break;
  }
